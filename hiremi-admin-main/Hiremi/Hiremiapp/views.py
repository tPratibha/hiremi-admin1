...
def superuser_login(request):
    if request.method == 'POST':
        ...
        if user is not None and user.is_superuser:
            ...
            messages.error(
                request, 'Invalid username or password for superuser.')
    return render(request, 'index.html')
...
def dashboard(request):
    ...
    state_counts = {state: {'total': 0, 'verified': 0, 'unverified': 0}
    ...
    if response.status_code == 200:
        data1 = response.json()
        verified_data = [
            entry for entry in data1 if entry.get('is_paid') == True]
    ...
    verified_count = sum(
        1 for registration in data if registration.get('verified') == True)
    # Count total unverified registrations
    unverified_count = sum(
        1 for registration in data if registration.get('verified') == False)
    ...
    current_month_registrations = sum(
        ...
        if 'time_end' in registration and
           ...
           datetime.strptime(
               registration['time_end'], '%Y-%m-%dT%H:%M:%S.%fZ').year == current_year
    )
...
def dashboard1(request):
    ...
    # This should be 'state', not 'state_filter'
    state_filter = request.GET.get('state', '')
    # This should be 'birth_state', not 'birth_state_filter'
    birth_state_filter = request.GET.get('birth_state', '')
    # This should be 'gender', not 'gender_filter'
    ...
    if college_filter:
        data = [item for item in data if college_filter.lower(
        ) in item.get('college_name', '').lower()]
    if branch_filter:
        data = [item for item in data if str(
            branch_filter) == str(item.get('branch_name', ''))]
    if year_filter:
        data = [item for item in data if str(
            year_filter) == str(item.get('passing_year', ''))]
    if state_filter:
        data = [item for item in data if state_filter.lower(
        ) == item.get('college_state', '').lower()]
    if birth_state_filter:
        data = [item for item in data if birth_state_filter.lower() ==
                item.get('date_of_birth', '').lower()]
    if gender_filter:
        data = [item for item in data if gender_filter.lower() ==
                item.get('gender', '').lower()]
    if status_filter:
        data = [item for item in data if isinstance(item.get('verified'), str) and status_filter.lower(
        ) in [status.strip().lower() for status in item.get('verified', '').split(',')]]
...
def view_Info1(request, pk):
    print(pk)
    data = requests.get(
        f'http://13.127.81.177:8000/api/registers/{pk}/').json()
...
def dashboard2(request):
    ...
    if response.status_code == 200:
        data = response.json()
        verified_data = [
            entry for entry in data if entry.get('verified') == True]
    else:
        verified_data = []
    ...
    if college_filter:
        filtered_data = [item for item in filtered_data if college_filter.lower(
        ) in item.get('college_name', '').lower()]
    if branch_filter:
        filtered_data = [item for item in filtered_data if branch_filter.lower(
        ) in item.get('branch_name', '').lower()]
    if year_filter:
        filtered_data = [item for item in filtered_data if str(
            year_filter) == str(item.get('passing_year', ''))]
    if state_filter:
        filtered_data = [item for item in filtered_data if state_filter.lower(
        ) == item.get('college_state', '').lower()]
    if birth_state_filter:
        filtered_data = [item for item in filtered_data if birth_state_filter.lower(
        ) == item.get('birth_state', '').lower()]
    if gender_filter:
        filtered_data = [item for item in filtered_data if gender_filter.lower(
        ) == item.get('gender', '').lower()]
    if status_filter:
        filtered_data = [item for item in filtered_data if status_filter.lower(
        ) in item.get('status', '').lower()]

    if name_query:
        filtered_data = [item for item in filtered_data if name_query in item.get(
            'full_name', '').lower()]
    if email_query:
        filtered_data = [
            item for item in filtered_data if email_query in item.get('email', '').lower()]
...
def dashboard4(request):
    ...
    if response.status_code == 200:
        data = response.json()
        verified_data = [
            entry for entry in data if entry.get('verified') == False]
...
def internship(request):
    data = requests.get(
        'http://13.127.81.177:8000/api/internship-applications/').json()
...
def intern_applied(request):
    internship_response = requests.get(
        'http://13.127.81.177:8000/api/internship-applications/')
    ...
    internship_profiles = [internship.get(
        'Internship_profile') for internship in internship_data]

    registers_response = requests.get(
        'http://13.127.81.177:8000/api/registers/')
...
def intern_info(request, pk):
    print(pk)
    data = requests.get(
        f'http://13.127.81.177:8000/api/internship-applications/{pk}/').json()
...
def intern_dash2(request):
    ...
    if response.status_code == 200:
        data = response.json()
        verified_data = [entry for entry in data if entry.get(
...
def intern_dash4(request):
    return render(request, 'intern_dash4.html')


def intern_dash5(request):
    return render(request, 'intern_dash5.html')


def Select_intern(request, pk):
    update_endpoint = f'http://13.127.81.177:8000/api/internship-applications/{
        pk}/'
...
def mentor_info1(request, pk):
    data = requests.get(
        f'http://13.127.81.177:8000/api/mentorship/{pk}/').json()
...
def corporate_training(request):
    data = requests.get(
        'http://13.127.81.177:8000/api/corporatetraining/').json()
...
def corporate_info1(request, pk):
    data = requests.get(
        f'http://13.127.81.177:8000/api/corporatetraining/{pk}/').json()
...
def fresher(request):
    data = requests.get(
        'http://13.127.81.177:8000/api/job-applications/').json()
...
def fresher_info1(request, pk):
    data = requests.get(
        f'http://13.127.81.177:8000/api/job-applications/{pk}/').json()
